{{-- resources/views/tenant/lessons/edit.blade.php --}}
...
<div class="mx-auto animate-fade-in">
    <div class="mb-8">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl sm:text-3xl font-bold text-white">Modifier la leçon</h1>
    ...
    <form action="{{ route('lessons.update', [app('tenant')->name, $lesson->id]) }}" method="POST">
        @csrf
        @method('PUT')
            ...
            <div class="bg-gray-900 border border-gray-800 rounded-xl p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="date" name="lesson_date" required value="{{ old('lesson_date', $lesson->lesson_date instanceof \Carbon\Carbon ? $lesson->lesson_date->format('Y-m-d') : $lesson->lesson_date) }}"
                    ...
                    <!-- <div>
                        ...
                        <input type="time" name="start_time" value="{{ old('start_time', $lesson->start_time ? (\Carbon\Carbon::parse($lesson->start_time)->format('H:i')) : '') }}"
                               class="w-full bg-gray-800 border border-gray-700 rounded-lg px-4 py-3 text-white">
                        @error('start_time')
                        <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="time" name="end_time" value="{{ old('end_time', $lesson->end_time ? (\Carbon\Carbon::parse($lesson->end_time)->format('H:i')) : '') }}"
                               class="w-full bg-gray-800 border border-gray-700 rounded-lg px-4 py-3 text-white">
                        @error('end_time')
                        <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <div id="objectives-container">
                            @php
                                ...
                                $objectives = old('objectives', $lesson->objectives ?? []);
                            ...
                            @empty
                                <div class="flex gap-2 mb-2">
                                    <input type="text" name="objectives[]" class="flex-1 bg-gray-800 border border-gray-700 rounded-lg px-4 py-2 text-white" placeholder="Objectif 1">
                                    <button type="button" class="remove-objective text-red-400 hover:text-red-300">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
                                        </svg>
                    ...
                    <div>
                        ...
                        <div id="resources-container">
                            @php
                                ...
                                $resources = old('resources', $lesson->resources ?? []);
