{{-- resources/views/tenant/homeworks/edit.blade.php --}}
            ...
            <div class="bg-gray-900 border border-gray-800 rounded-xl p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" name="title" required value="{{ old('title', $homework->title) }}"
                    ...
                    <div>
                        ...
                        <select name="type" required class="w-full bg-gray-800 border border-gray-700 rounded-lg px-4 py-3 text-white">
                            <option value="homework" {{ old('type', $homework->type) == 'homework' ? 'selected' : '' }}>Devoir</option>
                            <option value="project" {{ old('type', $homework->type) == 'project' ? 'selected' : '' }}>Projet</option>
                            <option value="research" {{ old('type', $homework->type) == 'research' ? 'selected' : '' }}>Recherche</option>
                        </select>
                        @error('type')
                        <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <select name="status" required class="w-full bg-gray-800 border border-gray-700 rounded-lg px-4 py-3 text-white">
                            <option value="active" {{ old('status', $homework->status) == 'active' ? 'selected' : '' }}>Actif</option>
                            <option value="expired" {{ old('status', $homework->status) == 'expired' ? 'selected' : '' }}>Expiré</option>
                            <option value="cancelled" {{ old('status', $homework->status) == 'cancelled' ? 'selected' : '' }}>Annulé</option>
                        </select>
                        @error('status')
                        <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="number" name="max_score" required value="{{ old('max_score', $homework->max_score) }}"
                               class="w-full bg-gray-800 border border-gray-700 rounded-lg px-4 py-3 text-white"
                               min="1" max="100">
                        @error('max_score')
                        <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <input type="date" name="due_date" required value="{{ old('due_date', $homework->due_date->format('Y-m-d')) }}"
                               class="w-full bg-gray-800 border border-gray-700 rounded-lg px-4 py-3 text-white">
                        @error('due_date')
                        <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="time" name="due_time" value="{{ old('due_time', $homework->due_time ? date('H:i', strtotime($homework->due_time)) : '') }}"
                               class="w-full bg-gray-800 border border-gray-700 rounded-lg px-4 py-3 text-white">
                        @error('due_time')
                        <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                        @enderror
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <textarea name="description" rows="4" required
                                  class="w-full bg-gray-800 border border-gray-700 rounded-lg px-4 py-3 text-white">{{ old('description', $homework->description) }}</textarea>
                        @error('description')
                        <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="instructions" rows="4"
                                  class="w-full bg-gray-800 border border-gray-700 rounded-lg px-4 py-3 text-white">{{ old('instructions', $homework->instructions) }}</textarea>
                        @error('instructions')
                        <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                        @enderror
                    </div>
