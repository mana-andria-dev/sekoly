{{-- resources/views/tenant/exams/create.blade.php --}}
...
<div class="mx-auto animate-fade-in">
    <div class="mb-8">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl sm:text-3xl font-bold text-white">Nouvel examen</h1>
                <p class="text-gray-400 text-sm mt-1">Créez un nouvel examen, contrôle ou évaluation</p>
            </div>
    ...
    <form action="{{ route('exams.store', app('tenant')->name) }}" method="POST">
        @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="type" required class="w-full bg-gray-800 border border-gray-700 rounded-lg px-4 py-3 text-white">
                            ...
                            <option value="trimester" {{ old('type') == 'trimester' ? 'selected' : '' }}>Trimestriel</option>
                            <option value="semester" {{ old('type') == 'semester' ? 'selected' : '' }}>Semestriel</option>
                    ...
                    <div>
                        ...
                        <input type="number" name="max_score" required value="{{ old('max_score', 20) }}" step="0.5"
                               class="w-full bg-gray-800 border border-gray-700 rounded-lg px-4 py-3 text-white"
                               min="1" max="100">
                        @error('max_score')
                        <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <input type="number" name="coefficient" required value="{{ old('coefficient', 1) }}" step="0.5"
                               class="w-full bg-gray-800 border border-gray-700 rounded-lg px-4 py-3 text-white"
                               min="0.5" max="5">
                        @error('coefficient')
                        <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
